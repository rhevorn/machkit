...
- 支持 npm、Python、Cargo、Xcode、Homebrew、CocoaPods、SwiftPM、Yarn、Gradle、Android 和 Apple 模拟器等常见开发环境。
- 检查对应 App 已不存在的缓存、偏好设置、应用数据、保存状态、日志和沙盒容器；卸载残留不会默认选中。
- 扫描过程会显示当前规则、路径和安全说明；结果页汇总可安全清理与待确认空间。
...
- 显示 GPU、统一内存和建议工作集大小；不使用不稳定接口伪造 GPU 占用率。
