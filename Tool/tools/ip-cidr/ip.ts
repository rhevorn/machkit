...
export type IPv4Parsed = {
  octets: number[];
  integer: number;
};

export type IPv6Expanded = {
  groups: string[];
  zone: string;
};

export type IPInspectFailure = {
  ok: false;
  error: string;
};

export type IPv4InspectResult = {
  ok: true;
  version: 4;
  address: string;
  integer: string;
...
export type IPInspectResult = IPInspectFailure | IPv4InspectResult | IPv6InspectResult;

export function parseIPv4(input: unknown): IPv4Parsed | null {
...
export function formatIPv4(integer: number): string {
  const n = integer >>> 0;
  return `${(n >>> 24) & 255}.${(n >>> 16) & 255}.${(n >>> 8) & 255}.${n & 255}`;
}

function splitZone(text: string): { head: string; zone: string } {
  const idx = text.indexOf("%");
  if (idx === -1) return { head: text, zone: "" };
  return { head: text.slice(0, idx), zone: text.slice(idx + 1) };
}

export function expandIPv6(input: unknown): IPv6Expanded | null {
...
function ipv4Class(octets: number[]): string {
...
function ipv4Kind(octets: number[], integer: number): string {
...
function ipv6Kind(groups: string[]): string {
...
export function inspectIP(input: unknown): IPInspectResult {
  const text = String(input ?? "").trim();
  ...
  const result: IPv6InspectResult = {
    ok: true,
    version: 6,
