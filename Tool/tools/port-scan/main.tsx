...
function PortScanTool() {
  ...
  const [preset, setPreset] = useState<PresetSelection>("common");
  const [ports, setPorts] = useState<string>(presets.common);
  ...
  function selectPreset(next: string): void {
    const value = next as PresetSelection;
    setPreset(value);
    if (value !== "custom") setPorts(presets[value]);
  }
  ...
  const status: { tone: StatusTone; label: string } = running
      ...
      <ToolContent className="flex flex-col gap-3 pt-3 pb-4">
        <ToolToolbar className="flex-wrap gap-2">
          <Input
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter" && !running) void startScan();
            }}
