...
export type IdFormat = (typeof formats)[number];
...
export type UuidNamespaceKey = keyof typeof uuidNamespaces;
...
export type StringAlphabetOptions = {
  upper?: boolean;
  lower?: boolean;
  digits?: boolean;
  symbols?: boolean;
...
export type GenerateStringOptions = StringAlphabetOptions & {
  length?: number;
};

export type GenerateIdOptions = UuidFormatOptions &
...
export type ValidateIdResult = ValidateIdSuccess | ValidateIdFailure;
...
function uuidTimestamp(now = Date.now()): bigint {
...
export function formatUuid(value: unknown, { uppercase = false, hyphens = true }: UuidFormatOptions = {}): string {
  const compact = String(value).replace(/-/g, "").toLowerCase();
...
export function generateUuidV6(now = Date.now()): string {
  const timestamp = uuidTimestamp(now);
  const bytes = randomBytes(16);
...
async function generateNameBasedUuid(version: number, namespace: unknown, name: unknown): Promise<string> {
  const namespaceBytes = parseUuidBytes(String(namespace || uuidNamespaces.dns));
...
export function generateUuidV3(namespace: unknown, name: unknown): Promise<string> {
  return generateNameBasedUuid(3, namespace, name);
}

export function generateUuidV5(namespace: unknown, name: unknown): Promise<string> {
...
export function generateNanoId(length: unknown = defaultNanoLength): string {
...
export function generateHex(byteLength: unknown = defaultHexBytes, { uppercase = false }: { uppercase?: boolean } = {}): string {
...
export async function generateId(format: IdFormat | string, options: GenerateIdOptions = {}): Promise<string> {
  switch (format) {
    case "uuid-v1":
...
function parseUuid(value: string): ValidateIdResult | null {
  ...
  const variant = variantNibble >= 8 && variantNibble <= 11
    ...
    : variantNibble >= 12
      ? "Microsoft"
      : "NCS";
  const ok = version >= 1 && version <= 8 && variant === "RFC 4122";
  if (ok) {
    return {
      ok: true,
      kind: "uuid",
...
function parseUlid(value: string): ValidateIdSuccess | null {
...
function parseObjectId(value: string): ValidateIdSuccess | null {
...
function parseHex(value: string): ValidateIdSuccess | null {
...
export function validateId(raw: unknown): ValidateIdResult {
  ...
  if (uuid) {
    return uuid.ok
      ...
      : {
          ...uuid,
          error: (uuid.version ?? 0) < 1 || (uuid.version ?? 0) > 8 ? "uuid-version" : "uuid-variant",
        };
  }
...
export function validateIds(text: unknown): { line: number; input: string; result: ValidateIdResult }[] {
