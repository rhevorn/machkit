...
test("shifts fixed durations and calendar months in a zone", () => {
  ...
  const thirtyDaysAgo = shiftMilliseconds(base, 30, "days", "ago", "Asia/Shanghai");
  assert.ok(thirtyDaysAgo != null);
  assert.equal(localDateTimeValue(thirtyDaysAgo, "Asia/Shanghai"), "2026-07-12T16:30:45");
  const oneMonthLater = shiftMilliseconds(base, 1, "months", "later", "Asia/Shanghai");
  assert.ok(oneMonthLater != null);
  assert.equal(localDateTimeValue(oneMonthLater, "Asia/Shanghai"), "2026-09-11T16:30:45");
  const febClamp = shiftMilliseconds(Date.UTC(2026, 0, 31, 4, 0, 0), 1, "months", "later", "Asia/Shanghai");
  assert.ok(febClamp != null);
  assert.equal(localDateTimeValue(febClamp, "Asia/Shanghai"), "2026-02-28T12:00:00");
});
