...
function createCalcStep(operator = "add", amount = "1", unit = "days") {
  return {
...
function FormatRow({ label, value, copyLabel, placeholder }) {
...
function FormatGrid({ items, text }) {
  return (
    <div>
...
function resultFormatItems(milliseconds, timeZone, locale, text, unitLabel, unit) {
...
function TimestampTool() {
  ...
  const [calcSteps, setCalcSteps] = useState(() => [createCalcStep("add", "30", "days")]);
  ...
  const relativeUnitOptions = useMemo(
    () => relativeUnits.map((id) => ({ value: id, label: text[id] || id })),
    [text],
  );
  ...
  const applyMoment = (nextMilliseconds) => {
  ...
  const changeUnit = (nextUnit) => {
    setUnit(nextUnit);
    setTimestampInput(timestampFromMilliseconds(milliseconds, nextUnit));
  };

  const onTimestampChange = (raw) => {
              ...
              <div className="space-y-2">
                {calcSteps.map((step) => (
                  ...
                  >
                    <SegmentedControl
                      value={step.operator}
                      onChange={(operator) => updateCalcStep(step.id, { operator })}
                      label={text.operator}
                      options={[
                    ...
                    <AmountStepper
                      ...
                      onChange={(amount) => updateCalcStep(step.id, { amount })}
                      onNudge={(delta) => nudgeCalcAmount(step.id, delta)}
                    />
                    <SelectControl
                      value={step.unit}
                      options={relativeUnitOptions}
                      onChange={(nextUnit) => updateCalcStep(step.id, { unit: nextUnit })}
