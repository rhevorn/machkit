...
export function rgbToHsl({ r, g, b }: Pick<RGB, "r" | "g" | "b">): HSL {
  const rn = r / 255;
  const gn = g / 255;
  const bn = b / 255;
...
export function rgbToHsv({ r, g, b }: Pick<RGB, "r" | "g" | "b">): HSV {
  const rn = r / 255;
  const gn = g / 255;
  const bn = b / 255;
  const max = Math.max(rn, gn, bn);
...
export function hslToRgb({ h, s, l }: HSL): Pick<RGB, "r" | "g" | "b"> {
...
export function hsvToRgb({ h, s, v }: HSV): Pick<RGB, "r" | "g" | "b"> {
  const sat = clamp(s, 0, 100) / 100;
...
function relativeLuminance({ r, g, b }: Pick<RGB, "r" | "g" | "b">): number {
...
function parseHex(raw: string): Required<RGB> | null {
  ...
  const expand = (chunk: string) => chunk.split("").map((ch) => ch + ch).join("");
...
function parseHsl(raw: string): Required<RGB> | null {
  const match = raw.match(
...
export function parseColor(input: unknown): ColorParseResult {
  const raw = String(input ?? "").trim();
  if (!raw) return { ok: false, error: "empty" };
  if (raw.length > maxColorInput) return { ok: false, error: "too-large" };

  let rgb: Required<RGB> | null = null;
  ...
  return {
    ok: true,
    rgb: { r: rgb.r, g: rgb.g, b: rgb.b, a: alpha },
    hsl,
    hsv,
    hex,
    formats: {
...
export function colorFromChannels({ r, g, b, a = 1 }: RGB): ColorParseResult {
