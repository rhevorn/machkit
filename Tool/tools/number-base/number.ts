...
export function parseFloatAmount(input: any) {
  ...
  if (!/^-?\d+(\.\d+)?([eE][+-]?\d+)?$/.test(raw)) return { ok: false as const, error: "invalid", value: null };
  const value = Number(raw);
  if (!Number.isFinite(value)) return { ok: false as const, error: "invalid", value: null };
...
export function parseInteger(input: any, base: number | string) {
  ...
  if (![2, 8, 10, 16].includes(radix)) return { ok: false as const, error: "invalid-base", value: null };
  ...
  if (negative && radix !== 10) return { ok: false as const, error: "invalid", value: null };
  const body = negative ? text.slice(1) : text;
  if (!pattern.test(negative ? text : body) && !(negative && /^-?\d+$/.test(text))) {
    return { ok: false as const, error: "invalid", value: null };
  }
  if (radix !== 10 && !pattern.test(body)) return { ok: false as const, error: "invalid", value: null };
  if (radix === 10 && !/^-?\d+$/.test(text)) return { ok: false as const, error: "invalid", value: null };

  try {
    const prefix = radix === 16 ? "0x" : radix === 2 ? "0b" : radix === 8 ? "0o" : "";
    const value = BigInt(radix === 10 ? text : `${prefix}${body}`);
    return { ok: true as const, error: null, value, base: radix };
  } catch {
    return { ok: false as const, error: "invalid", value: null };
  }
}

export function formatInteger(value: bigint, base: number) {
...
export function parseByteAmount(input: any, unit: string = "B") {
  ...
  if (!meta) return { ok: false as const, error: "invalid-unit", bytes: null };

  const [whole, fraction = ""] = raw.split(".");
  const wholePart = BigInt(whole || "0") * meta.factor;
  if (!fraction) return { ok: true as const, error: null, bytes: wholePart };
...
export function convertLinear(input: any, unitId: string, category: string) {
  ...
  if (!from) return { ok: false as const, error: "invalid-unit", formats: null, rows: null };
