...
test("converts decimal to other bases", () => {
  ...
  if (!result.ok || !result.formats) throw new Error("expected ok");
...
test("formats exabyte-scale bytes without Number precision loss", () => {
  const result = convertBytes("1", "EB");
  assert.equal(result.ok, true);
  if (!result.ok || !result.formats) throw new Error("expected ok");
  assert.equal(result.formats.B, "1000000000000000000");
  assert.equal(result.formats.PB, "1000");
  assert.equal(result.formats.EB, "1");
  // Fractional conversion must stay exact for integer multiples of the unit.
  assert.equal(result.formats.EiB.startsWith("0.867"), true);
  // A value past Number.MAX_SAFE_INTEGER must not collapse via IEEE rounding.
  const huge = convertBytes("9007199254740993", "B");
  assert.equal(huge.ok, true);
  if (!huge.ok || !huge.formats) throw new Error("expected ok");
  assert.equal(huge.formats.B, "9007199254740993");
  assert.match(huge.formats.KiB, /^8796093022208/);
});
...
test("routes categories and lists units", () => {
  ...
  if (!mass.ok || !mass.formats) throw new Error("expected ok");
  assert.equal((mass.formats as Record<string, string>).g, "1000");

  const area = convertCategory("area", "1", "ha");
  assert.equal(area.ok, true);
  if (!area.ok || !area.formats) throw new Error("expected ok");
  assert.equal((area.formats as Record<string, string>).m2, "10000");
