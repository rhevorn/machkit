...
export const bodyModes = Object.freeze(["none", "raw", "urlencoded", "formdata"] as const);
...
export type ParseCurlResult =
  | { ok: true; error: null; request: CurlRequest }
  | { ok: false; error: string; request: null };

export type FormatRawBodyResult =
  | { ok: true; text: string; kind: "json" | "xml"; error?: undefined }
  | { ok: false; error: string; text: string; kind?: undefined };
...
function isBodyMode(value: string): value is BodyMode {
  return (bodyModes as readonly string[]).includes(value);
}
...
export function createFormField(key = "", value = "", kind: FormFieldKind | string = "text"): FormField {
  return {
    id: nid("f"),
    key,
    value,
...
export function createEmptyRequest(): CurlRequest {
  return {
    method: "GET",
...
function parseHeaderLine(line: string): KeyValuePair {
...
function applyQuery(url: string, query: KeyValuePair[] | null | undefined): string {
...
function headerMap(headers: KeyValuePair[] | null | undefined): Map<string, { key: string; value: string }> {
...
function hasHeader(headers: KeyValuePair[] | null | undefined, name: string): boolean {
  return headerMap(headers).has(String(name).toLowerCase());
}

function parseFormFieldToken(token: unknown, { forceText = false }: { forceText?: boolean } = {}): FormField {
...
function looksUrlEncoded(text: unknown): boolean {
...
function parseUrlEncodedBody(text: unknown): FormField[] {
...
export function buildCurl(request: Partial<CurlRequest> | null | undefined): string {
  const methodValue = String(request?.method || "");
  const method = isHttpMethod(methodValue) ? methodValue : "GET";
  const bodyModeValue = String(request?.bodyMode || "");
  const bodyMode = isBodyMode(bodyModeValue) ? bodyModeValue : "none";
...
export function parseCurl(input: unknown): ParseCurlResult {
  ...
  const urlEncodeParts: FormField[] = [];
  const formParts: FormField[] = [];
  ...
  for (let i = 1; i < tokens.length; i += 1) {
    ...
    if (token === "-X" || token === "--request") {
      ...
      request.method = isHttpMethod(value) ? value : "GET";
      methodSet = true;
      continue;
    }
...
export function buildFetch(request: Partial<CurlRequest> | null | undefined): string {
  const methodValue = String(request?.method || "");
  const method = isHttpMethod(methodValue) ? methodValue : "GET";
  const bodyModeValue = String(request?.bodyMode || "");
  const bodyMode = isBodyMode(bodyModeValue) ? bodyModeValue : "none";
  ...
  if (bodyMode === "urlencoded" && fields.length) {
    ...
    const init: Record<string, unknown> = { method, headers, body: "BODY" };
...
export function formatRawBody(input: unknown): FormatRawBodyResult {
