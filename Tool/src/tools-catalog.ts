...
export const toolMeta = {
  ...
  "json-formatter": {
    ...
    description: {
      en: "Format, minify, sort, and inspect JSON values",
      "zh-Hans": "格式化、压缩、排序并查看 JSON 值",
      "zh-Hant": "格式化、壓縮、排序並查看 JSON 值",
      ja: "JSON の整形・圧縮・キー並び替え・値の確認",
      ko: "JSON 포맷, 압축, 키 정렬 및 값 검사",
      es: "Formatea, minimiza, ordena e inspecciona valores JSON",
      fr: "Formatez, minifiez, triez et inspectez les valeurs JSON",
      de: "JSON-Werte formatieren, komprimieren, sortieren und prüfen",
      "pt-BR": "Formate, compacte, ordene e inspecione valores JSON",
      ru: "Форматирование, сжатие, сортировка и просмотр значений JSON",
    },
  },
  codec: {
    title: {
...
export function discoverToolIds(): string[] {
  return Object.keys(pageModules)
    .map((path) => path.match(/\/tools\/([^/]+)\//)?.[1])
    .filter((id): id is string => typeof id === "string" && !id.startsWith("_"))
    .sort((left, right) => {
...
function pickLocale(map: Record<string, string> | undefined, locale: string): string {
  if (!map) return "";
...
export function listTools(locale = "en") {
  return discoverToolIds().map((id) => {
    const meta = (toolMeta as Record<string, { title?: Record<string, string>; description?: Record<string, string> }>)[id];
    return {
      id,
