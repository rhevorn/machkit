...
type CopyToast = { id: number; ok: boolean };
...
export function ToolPage({ title, adaptiveHeight = true, children }: ToolPageProps) {
  ...
  React.useEffect(() => {
    ...
    const content =
      pageRef.current?.querySelector(":scope > [data-machkit-tool-content]") ?? pageRef.current;
...
function CopyFeedbackToast() {
  ...
  const [toast, setToast] = React.useState<CopyToast | null>(null);
  ...
  React.useEffect(() => {
    const showToast = (event: Event) => {
      ...
      setToast({ id: Date.now(), ok: detail?.ok !== false });
  ...
  const labels = copiedLabels[locale as keyof typeof copiedLabels] || copiedLabels.en;
