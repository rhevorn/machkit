...
test("builds a direct macOS zip URL from a release tag", () => {
  const download = releaseFromTag("v1.2.1");
  assert.deepEqual(download, {
    tag: "v1.2.1",
    version: "1.2.1",
    downloadURL: "https://github.com/rhevorn/machkit/releases/download/v1.2.1/MachKit-1.2.1-macOS.zip",
  });
});
...
test("resolves the MachKit release asset from the latest tag", () => {
  ...
  assert.equal(
    ...
    "https://github.com/rhevorn/machkit/releases/download/v1.2.1/MachKit-1.2.1-macOS.zip",
  );
});
...
test("uses the known direct download when release metadata is invalid", () => {
  ...
  assert.equal(fallbackRelease.tag, "v2.3.0");
  assert.match(fallbackRelease.downloadURL, /MachKit-2\.3\.0-macOS\.zip$/);
});
